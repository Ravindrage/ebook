...
if (isset($_GET["preview"]) and ($_GET["preview"]>"")){
	...
	$sqlServicesPreview = mysqli_query($con,"SELECT id, title, icon, image, content, link FROM services WHERE id='$pagePreviewId'");
...
<?php
	...
	if (isset($_GET["newservice"]) OR isset($_GET["editservice"])) {
		$serviceMsg="";
	...
	if (isset($_GET["editservice"])) {
			...
			if (!empty($_POST["service_title"])) {
				...
				mysqli_query($con,$servicesUpdate);
			...
			$sqlServices = mysqli_query($con,"SELECT id, title, icon, image, content, link, active, datetime FROM services WHERE id='$theserviceId'");
		...
		//Create new service
			...
			if (!empty($_POST["service_title"])) {
				...
				mysqli_query($con,$servicesInsert);
	...
	<form role="serviceForm" method="post" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <select class="form-control input-sm" name="service_status">
                <option value="1" <?php if(isset($_GET["editservice"])){echo $selActive1;} ?>>Active</option>
                <option value="0" <?php if(isset($_GET["editservice"])){echo $selActive0;} ?>>Draft</option>
            </select>
        </div>
		<div class="form-group">
			<label><?php echo $serviceLabel; ?></label>
			<input class="form-control input-sm" name="service_title" value="<?php if(isset($_GET["editservice"])){echo $row['title'];} ?>" placeholder="Service Title">
		</div>
		<hr/>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<select class="form-control input-sm" name="service_icon_select" id="service_icon_select">
				...
				<?php
					$sqlServicesIcon = mysqli_query($con,"SELECT icon FROM services_icons ORDER BY icon ASC");
		...
		<div class="form-group">
			...
			<select class="form-control input-sm" name="service_link">
				...
				<?php
					$pagesStr="";
					$sqlServicesLink = mysqli_query($con,"SELECT id, title FROM pages WHERE active=1 ORDER BY title ASC");
		...
		<div class="form-group">
			...
			<textarea class="form-control input-sm" rows="3" name="service_content" placeholder="Text" maxlength="255"><?php if(isset($_GET["editservice"])){echo $row['content'];} ?></textarea>
		</div>
        <div class="form-group">
			<span><?php if(isset($_GET["editservice"])){echo "Updated: ".date('m-d-Y, H:i:s',strtotime($row['datetime']));} ?></span>
		</div>
...
<?php
		...
		if(isset($_GET["deleteservice"])and !empty($_GET["deleteservice"])){ $delserviceId = $_GET["deleteservice"]; }
		if(isset($_GET["deletetitle"]) and !empty($_GET["deletetitle"])){ $delserviceTitle = $_GET["deletetitle"]; }
		if(isset($_GET["moveservice"]) and !empty($_GET["moveservice"])){ $moveserviceId = $_GET["moveservice"]; }
		if(isset($_GET["movetitle"]) and !empty($_GET["movetitle"])){ $moveserviceTitle = $_GET["movetitle"]; }
		
		
		//delete service
		if (isset($_GET["deleteservice"]) AND isset($_GET["deletetitle"]) AND !isset($_GET["confirm"])) {
			...
			echo $deleteMsg;
		} elseif (isset($_GET["deleteservice"]) AND isset($_GET["deletetitle"]) AND ($_GET["confirm"]=="yes")) {
			//delete service after clicking Yes
			$servicesDelete = "DELETE FROM services WHERE id='$delserviceId'";
			mysqli_query($con,$servicesDelete);
    ...
    $sqlSetup = mysqli_query($con,"SELECT servicesheading, servicescontent FROM setup");
    ...
    <?php 
		...
		?>
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
        		<?php 
					$sqlServices = mysqli_query($con,"SELECT id, title, icon, content, link, active FROM services ORDER BY datetime DESC");
