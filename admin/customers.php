...
include 'includes/header.php';
	...
	//Page preview
	if (isset($_FILES["fileToUpload"]["tmp_name"]) and ($_GET["preview"]>"")){
		$pagePreviewId=$_GET["preview"];
		$sqlcustomerPreview = mysqli_query($con,"SELECT id, image, name, link FROM customers WHERE id='$pagePreviewId'");
...
<?php
	...
	if (isset($_GET["newcustomer"]) OR isset($_GET["editcustomer"])) {
		$customerMsg="";
		
		//Update existing customer
		if (isset($_GET["editcustomer"]) and !empty($_GET["editcustomer"])) {
			...
			if (!empty($_POST["customer_name"])) {
				...
				mysqli_query($con,$customerUpdate);
			...
			$sqlcustomer = mysqli_query($con,"SELECT id, image, name, link, active, datetime FROM customers WHERE id='$thecustomerId'");
		...
		//Create new customer
			...
			if (!empty($_POST["customer_name"])) {
				...
				mysqli_query($con,$customerInsert);
				//echo $customerInsert;
	...
	<form role="customerForm" method="post" action="" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <select class="form-control input-sm" name="customer_status">
                <option value="1" <?php if(isset($_GET["editcustomer"])){echo $selActive1;} ?>>Active</option>
                <option value="0" <?php if(isset($_GET["editcustomer"])){echo $selActive0;} ?>>Draft</option>
            </select>
        </div>
        <hr/>
        <div class="form-group">
		...
		<div class="form-group">
			<label>Name</label>
			<input class="form-control input-sm" name="customer_name" value="<?php if(isset($_GET["editcustomer"])){echo $row['name'];} ?>" placeholder="Name">
		</div>
		<div class="form-group">
			<label>Link</label>
			<input class="form-control input-sm" name="customer_link" value="<?php if(isset($_GET["editcustomer"])){echo $row['link'];} ?>" placeholder="http://www.google.com">
		</div>
        <div class="form-group">
			<span><?php if(isset($_GET["editcustomer"])){echo "Updated: ".date('m-d-Y, H:i:s',strtotime($row['datetime']));} ?></span>
		</div>
...
<?php
		...
		$deleteConfirm="";
		$customerMsg="";
		if(isset($_GET["deletecustomer"]) and !empty($_GET["deletecustomer"]) ){ $delcustomerId = $_GET["deletecustomer"];}
		if(isset($_GET["deletename"]) and !empty($_GET["deletename"]) ){ $delcustomerName = $_GET["deletename"]; }  
		if(isset($_GET["deletename"]) and !empty($_GET["deletename"]) ){ $delcustomerName = $_GET["deletename"]; }  
		if(isset($_GET["movecustomer"]) and !empty($_GET["deletename"]) ){ $movecustomerId = $_GET["movecustomer"]; }  
		if(isset($_GET["movename"]) and !empty($_GET["deletename"]) ){ $movecustomerName = $_GET["movename"]; }  
		...
		//delete customer
		if (isset($_GET["deletecustomer"]) AND isset($_GET["deletename"]) AND !isset($_GET["confirm"])) {
			...
			echo $deleteMsg;
		} elseif (isset($_GET["deletecustomer"]) AND isset($_GET["deletename"]) AND ($_GET["confirm"]=="yes")) {
			//delete customer after clicking Yes
			$customerDelete = "DELETE FROM customers WHERE id='$delcustomerId'";
			mysqli_query($con,$customerDelete);
    ...
    $sqlSetup = mysqli_query($con,"SELECT customersheading, customerscontent FROM setup");
    ...
    <?php 
		...
		?>
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
        		<?php 
					$sqlcustomer = mysqli_query($con,"SELECT id, image, name, link, active FROM customers ORDER BY datetime DESC");
