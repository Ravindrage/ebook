...
if (isset($_GET["preview"]) and ($_GET["preview"]>"")){
	...
	$sqlPagePreview = mysqli_query($con,"SELECT id, title, image, content FROM pages WHERE id='$pagePreviewId'");
...
?>
   <div class="row">
	...
	<div class="row">
		<div class="col-lg-12">
<?php
	if (isset($_GET["newpage"]) OR isset($_GET["editpage"])) {
		//Upload function
		...
		if (isset($_GET["editpage"])) {
			...
			$sqlPages = mysqli_query($con,"SELECT id, title, image, content, active, datetime, image_align FROM pages WHERE id='$thePageId'");
		...
		//image algin status		
		if (isset($row['image_align']) and ($row['image_align']=="left")) {
	...
	<form role="pageForm" method="post" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <select class="form-control input-sm" name="page_status">
                <option value="1" <?php if(isset($_GET["editpage"]) and !empty($_GET["editpage"])){echo $selActive1;}?>>Active</option>
                <option value="0" <?php if(isset($_GET["editpage"]) and !empty($_GET["editpage"])){echo $selActive0;} ?>>Draft</option>
            </select>
        </div>
		<div class="form-group">
			<label><?php echo $pageLabel; ?></label>
			<input class="form-control input-sm" name="page_title" value="<?php if(isset($_GET["editpage"])){echo $row['title'];} ?>" placeholder="Page Title">
		</div>
		<hr/>
        <div class="form-group">
		...
		<div class="form-group">
			...
			<textarea class="form-control input-sm tinymce" rows="20" name="page_content" id="page_content"><?php if(isset($_GET["editpage"])){echo $row['content'];} ?></textarea>
		</div>
        <div class="form-group">
			<span><?php if(isset($_GET["editpage"])){echo "Updated: ".date('m-d-Y, H:i:s',strtotime($row['datetime']));} ?></span>
		</div>
...
<?php
		...
		if (isset($_GET["deletepage"]) AND !isset($_GET["deletepage"])) {  $delPageId = $_GET["deletepage"];  }
		if (isset($_GET["deletetitle"]) AND !isset($_GET["deletetitle"])) {  $delPageTitle = $_GET["deletetitle"];  }
		if (isset($_GET["movepage"]) AND !isset($_GET["movepage"])) {  $movePageId = $_GET["movepage"];  }
		if (isset($_GET["movetitle"]) AND !isset($_GET["movetitle"])) { $movePageTitle = $_GET["movetitle"]; }
		...
		//delete page
		if (isset($_GET["deletepage"]) AND isset($_GET["deletetitle"]) AND !isset($_GET["confirm"])) {
			...
			echo $deleteMsg;
		} elseif (isset($_GET["deletepage"]) AND isset($_GET["deletetitle"]) AND ($_GET["confirm"]=="yes")) {
			//delete page after clicking Yes
			$pageDelete = "DELETE FROM pages WHERE id='$delPageId'";
			mysqli_query($con,$pageDelete);
		...
		if (isset($_GET["movepage"]) AND isset($_GET["movetitle"])) {
			$pagesDateUpdate = "UPDATE pages SET datetime='".date("Y-m-d H:i:s")."' WHERE id='$movePageId'";
			mysqli_query($con,$pagesDateUpdate);
    ...
    $sqlSetup = mysqli_query($con,"SELECT pageheading FROM setup");
  ...
  <div class="modal-dialog">
	...
	?>
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
        <?php 
					$sqlPages = mysqli_query($con,"SELECT id, title, image, content, active FROM pages ORDER BY datetime DESC");
